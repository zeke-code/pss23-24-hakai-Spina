...
/**
 * A system that updates the velocity of player-controlled entities based on keyboard input.
 * It checks for left/right movement keys and adjusts the entity's horizontal velocity accordingly.
 */
        ...
        for (Entity entity : entities) {
            ...
            if (entity.hasComponent(InputComponent.class)) {
                ...
                entity.getComponent(VelocityComponent.class).ifPresent(velocity -> {
                    ...
                    if (inputManager.isKeyPressed(KeyCode.LEFT) || inputManager.isKeyPressed(KeyCode.A)) {
                        velocity.x = -PADDLE_SPEED;
                    }
                    if (inputManager.isKeyPressed(KeyCode.RIGHT) || inputManager.isKeyPressed(KeyCode.D)) {
                        velocity.x = PADDLE_SPEED;
                    }
                });
            }
